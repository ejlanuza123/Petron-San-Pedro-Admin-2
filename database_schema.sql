...
CREATE TABLE public.deliveries (
  ...
  order_id bigint,
  rider_id uuid,
  status text CHECK (status = ANY (ARRAY['assigned'::text, 'accepted'::text, 'picked_up'::text, 'out_for_delivery'::text, 'delivered'::text, 'failed'::text, 'declined'::text])),
...
CREATE TABLE public.orders (
  ...
  status text DEFAULT 'Pending'::text CHECK (status = ANY (ARRAY['Pending'::text, 'Processing'::text, 'Rider Picked Up the Order'::text, 'Out for Delivery'::text, 'Completed'::text, 'Cancelled'::text])),
  created_at timestamp with time zone DEFAULT now(),
