...
export default function AdminLogsViewer({ 
  ...
  const getActionColor = (action) => {
    if (action.includes('create')) return isDarkMode ? 'text-green-400 bg-green-900/30 border-green-700' : 'text-green-600 bg-green-50 border-green-200';
    if (action.includes('update')) return isDarkMode ? 'text-blue-400 bg-blue-900/30 border-blue-700' : 'text-blue-600 bg-blue-50 border-blue-200';
    if (action.includes('delete')) return isDarkMode ? 'text-red-400 bg-red-900/30 border-red-700' : 'text-red-600 bg-red-50 border-red-200';
    if (action.includes('assign')) return isDarkMode ? 'text-purple-400 bg-purple-900/30 border-purple-700' : 'text-purple-600 bg-purple-50 border-purple-200';
    return isDarkMode ? 'text-gray-400 bg-gray-800 border-gray-700' : 'text-gray-600 bg-gray-50 border-gray-200';
  };
  ...
  const renderChangesTable = (details, riderNameByIdLocal = {}) => {
    ...
    return (
      <div className={`mt-3 border rounded-lg overflow-hidden transition-colors duration-300 ${isDarkMode ? 'border-slate-600' : 'border-gray-200'}`}>
        <table className="w-full text-xs text-left">
          <thead className={`uppercase transition-colors duration-300 ${isDarkMode ? 'bg-slate-600 text-gray-300' : 'bg-gray-50 text-gray-600'}`}>
            <tr>
          ...
          <tbody className={`divide-y transition-colors duration-300 ${isDarkMode ? 'divide-slate-600 bg-slate-700' : 'divide-gray-100 bg-white'}`}>
            {changes.map(([key, val]) => {
              ...
              return (
                <tr key={key}>
                  <td className={`px-3 py-2 font-medium capitalize transition-colors duration-300 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                    {key.replace(/_/g, ' ')}
                  </td>
                  <td className={`px-3 py-2 transition-colors duration-300 ${isDarkMode ? 'text-red-400 bg-red-900/20' : 'text-red-600 bg-red-50/50'}`}>
                    {shouldDecorateAsProfileId
                      ? decorateRiderIdValue(val.from)
                      : String(val.from || 'Empty/None')}
                  </td>
                  <td className={`px-3 py-2 font-medium transition-colors duration-300 ${isDarkMode ? 'text-green-400 bg-green-900/20' : 'text-green-600 bg-green-50/50'}`}>
                    {shouldDecorateAsProfileId
  ...
  if (logs.length === 0) {
    return (
      <div className={`text-center py-12 rounded-lg border transition-colors duration-300 ${isDarkMode ? 'bg-slate-700/50 border-slate-600' : 'bg-gray-50 border-gray-200'} dashed`}>
        <p className={`font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>No audit logs found for these filters.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 sticky top-0 pb-2 z-10 transition-colors duration-300 ${isDarkMode ? 'bg-slate-800' : 'bg-white'}`}>
        ...
        <label className={`flex items-center gap-2 text-xs cursor-pointer transition-colors duration-300 ${isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-600 hover:text-gray-900'}`}>
          <input
            type="checkbox"
      ...
      <div className="flex items-center justify-end gap-2 text-xs">
        <button
          ...
          className={`px-3 py-1.5 border rounded-md transition-colors duration-300 disabled:opacity-40 disabled:cursor-not-allowed ${isDarkMode ? 'border-slate-600 text-gray-300 hover:bg-slate-700' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
        >
          {exporting ? 'Exporting...' : 'Export CSV'}
        </button>
        <span className={`transition-colors duration-300 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Page {currentPage} of {totalPages}</span>
        <button
          type="button"
      ...
      <div className="space-y-4">
        {logs.map((log) => (
          <div key={log.id} className={`border rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow transition-colors duration-300 ${isDarkMode ? 'bg-slate-700 border-slate-600' : 'bg-white border-gray-200'}`}>
            <div className="flex flex-wrap justify-between items-start gap-2 mb-3">
              <div className="flex items-center gap-2">
                <span className={`text-xs px-2.5 py-1 rounded-full border font-bold ${getActionColor(log.action)}`}>
                  {formatAction(log.action)}
                </span>
                <span className={`text-xs font-medium px-2 py-1 rounded transition-colors duration-300 ${isDarkMode ? 'text-gray-300 bg-slate-600' : 'text-gray-500 bg-gray-100'}`}>
              ...
              <span className={`text-xs font-medium transition-colors duration-300 ${isDarkMode ? 'text-gray-400' : 'text-gray-400'}`}>
            ...
            <p className={`text-sm font-medium mb-1 transition-colors duration-300 ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
              {(() => {
                const details = log.details || {};
            ...
            {showSystemDetails && log.details && (
              <div className={`mt-3 text-xs border p-3 rounded-lg overflow-x-auto transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-600' : 'bg-slate-50 border-slate-200'}`}>
